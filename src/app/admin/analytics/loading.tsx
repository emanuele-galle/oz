export default function AdminAnalyticsLoading() {
  return (
    <div className="animate-pulse">
      <div className="h-8 w-32 bg-stone-800 rounded mb-8" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {Array.from({ length: 2 }).map((_, i) => (
          <div key={i} className="bg-stone-900 border border-stone-800 rounded-lg p-6">
            <div className="h-4 w-40 bg-stone-800 rounded mb-4" />
            <div className="space-y-3">
              {Array.from({ length: 6 }).map((_, j) => (
                <div key={j} className="h-5 bg-stone-800 rounded" />
              ))}
            </div>
          </div>
        ))}
        <div className="bg-stone-900 border border-stone-800 rounded-lg p-6 lg:col-span-2">
          <div className="h-4 w-36 bg-stone-800 rounded mb-4" />
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="h-14 bg-stone-800 rounded" />
            ))}
          </div>
        </div>
      </div>
