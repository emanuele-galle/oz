'use client';

import React, { useState } from 'react';
import { motion } from 'framer-motion';
import { Button } from '@/components/ui';
import { useScrollReveal } from '@/hooks/useScrollAnimation';

export function NewsletterSection() {
  const [email, setEmail] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [message, setMessage] = useState('');

  const containerRef = useScrollReveal({ delay: 0 });

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsSubmitting(true);
    setMessage('');

    // TODO: Implement newsletter subscription logic
    // For now, just simulate a successful submission
    setTimeout(() => {
      setMessage('✓ Iscrizione confermata! Controlla la tua email.');
      setEmail('');
      setIsSubmitting(false);
    }, 1500);
  };

  return (
    <section className="section-padding bg-gradient-to-b from-midnight to-black border-t border-white/10">
      <div className="container-luxury">
        <div ref={containerRef as any} className="max-w-4xl mx-auto">
          {/* Newsletter Form */}
          <div className="glass-card p-12 mb-12">
            <div className="text-center mb-8">
              <h2 className="font-cinzel text-4xl md:text-5xl text-gold mb-4">
                Offerte Esclusive
              </h2>
              <div className="h-1 w-24 bg-gold mx-auto mb-6" />
              <p className="font-playfair text-lg text-white/70">
                Iscriviti alla newsletter e ricevi il <span className="text-gold">10% di sconto</span> sul primo ordine
              </p>
            </div>

            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4 mb-4">
              <input
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="La tua email"
                required
                disabled={isSubmitting}
                className="flex-1 px-6 py-4 bg-black/30 border border-white/20 rounded text-white placeholder:text-white/40 focus:outline-none focus:border-gold transition-colors font-inter"
              />
              <Button
                type="submit"
                variant="primary"
                size="lg"
                loading={isSubmitting}
                className="sm:w-auto w-full"
              >
                Iscriviti
              </Button>
            </form>

            {message && (
              <motion.p
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                className="text-center text-green-400 font-inter text-sm"
              >
                {message}
              </motion.p>
            )}

            <p className="text-center text-white/40 text-xs font-inter mt-4">
              Iscrivendoti accetti la nostra{' '}
              <a href="/privacy" className="text-gold hover:text-gold-light underline">
                Privacy Policy
              </a>
            </p>
          </div>

          {/* Contact Options */}
          <div className="text-center">
            <h3 className="font-cinzel text-2xl text-white mb-6">
              Hai domande? Contattaci
            </h3>
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
              {/* WhatsApp */}
              <a
                href="https://wa.me/393123456789?text=Ciao%2C%20vorrei%20informazioni%20sui%20vostri%20profumi"
                target="_blank"
                rel="noopener noreferrer"
                className="group flex items-center gap-3 px-8 py-4 bg-green-600/10 border border-green-600/30 rounded hover:bg-green-600/20 hover:border-green-600/50 transition-all"
              >
                <svg className="w-6 h-6 text-green-500" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                </svg>
                <span className="font-inter text-white group-hover:text-green-400 transition-colors">
                  WhatsApp
                </span>
              </a>

              {/* Email */}
              <a
                href="mailto:info@ozextrait.com?subject=Richiesta Informazioni"
                className="group flex items-center gap-3 px-8 py-4 bg-gold/10 border border-gold/30 rounded hover:bg-gold/20 hover:border-gold/50 transition-all"
              >
                <svg className="w-6 h-6 text-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
                <span className="font-inter text-white group-hover:text-gold-light transition-colors">
                  Email
                </span>
              </a>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
