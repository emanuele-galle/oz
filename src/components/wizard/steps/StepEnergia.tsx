...
import { stepEnergia } from '@/data/fragrance-wizard';
...
export function StepEnergia({ selected, onSelect, onNext, onBack }: StepEnergiaProps) {
  const auraColor = selected === 'luminosa'
    ...
    <div className="relative flex flex-col items-center justify-center min-h-[80vh] px-6 py-12">
      ...
      <div className="relative z-10 w-full max-w-2xl space-y-8">
        <div className="text-center space-y-3">
          ...
          >
            {stepEnergia.subtitle}
          </motion.p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {stepEnergia.options.map((option, i) => (
            <motion.button
              key={option.id}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
