 ...
 * Source: React Bits (adapted for OZ Extrait with gold-tinted colors)
 * Creates interactive fluid trails following cursor movement
 */
...
export function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      const halfFloatTexType = isWebGL2 ? gl.HALF_FLOAT : (halfFloat && (halfFloat as unknown as Record<string, number>).HALF_FLOAT_OES) || 0;
    ...
    function correctDelta(delta: number, isX: boolean) {
      const ar = canvas!.width / canvas!.height;
      return isX ? (ar < 1 ? delta * ar : delta) : (ar > 1 ? delta / ar : delta);
    }

    // Gold-tinted color generation for luxury feel
    function generateColor(): ColorRGB {
      const hue = 0.11 + Math.random() * 0.04; // Gold hue range (40-55 degrees)
      const c = HSVtoRGB(hue, 0.7 + Math.random() * 0.3, 0.8 + Math.random() * 0.2);
    ...
    const onFirstMouseMove = (e: MouseEvent) => { updateFrame(); updatePointerMoveData(pointers[0], scaleByPixelRatio(e.clientX), scaleByPixelRatio(e.clientY), generateColor()); document.body.removeEventListener('mousemove', onFirstMouseMove); };
    const onFirstTouchStart = (e: TouchEvent) => { updateFrame(); for (let i = 0; i < e.targetTouches.length; i++) updatePointerDownData(pointers[0], e.targetTouches[i].identifier, scaleByPixelRatio(e.targetTouches[i].clientX), scaleByPixelRatio(e.targetTouches[i].clientY)); document.body.removeEventListener('touchstart', onFirstTouchStart); };
